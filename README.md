...
Pi 风格的 Agent Core/Harness SDK：`pi/ai` 提供公共消息与 Provider 抽象，其余目录按域拆分为工具执行、Run 治理、上下文组装、沙箱与观测等能力，由上层业务组合成完整的 Agent 运行时。

## 目录结构

```text
pi/
├── ai             公共消息模型、内容块、Usage、工具定义与统一 Provider 抽象
│   └── providers  模型平台适配器（OpenAI、Anthropic 等）与 Provider 配置
├── context        Run 上下文组装：System Prompt、剪枝与压缩
├── error          全域稳定错误码与错误值框架
...
- 域包只依赖 `pi/ai` 和自己的子包（`loopdetect` 额外允许被 `governor` 引用），不反向依赖 `pi` 包根的 Agent Core。
